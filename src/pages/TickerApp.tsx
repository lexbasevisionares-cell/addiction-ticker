...
import { Language, Currency, detectLanguage, detectCurrency } from '../utils/i18n';
...
export default function TickerApp() {
    ...
    try {
      ...
      if (savedSettings && savedState) {
        ...
        if (!parsedSettings.language) {
          parsedSettings.language = detectLanguage();
        }
        if (!parsedSettings.currency) {
          parsedSettings.currency = detectCurrency();
        }
  ...
  const activeLang = settings?.language || detectLanguage();
  const activeCur = settings?.currency || detectCurrency();
